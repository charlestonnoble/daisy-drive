function [T,D_prime,R_prime] = ...
    simulate_resistance(n,d0,h,c,d,s,t_max,cutoff_bool,cont_rel)
...
if nargin == 8 % If everything except continuous release specified, make 0
    cont_rel = 0;
end
...
[D_prime,R_prime] = convert_to_allele_frequencies(n, genotypes, Y);

if plot_bool
    plot_sims(T,D_prime,R_prime(:,2:end));
end
...
str_cell = {};
for i = 1:n_D
    pl_cell(end+1) = plot(ax,T,D(:,i),'-','Color',D_colors(i,:));
    str_cell{end+1} = ['Drive allele ' num2str(i)];
end
for i = 1:n_R
    pl_cell(end+1) = plot(ax,T,R(:,i),'-','Color',R_colors(i+1,:));
    str_cell{end+1} = ['Resistant allele ' num2str(i+1)];
end
ylabel('Allele frequency')
xlabel('Generations')
legend(pl_cell([n_D:-1:1, end:-1:n_D+1]),str_cell{[n_D:-1:1, end:-1:n_D+1]})
